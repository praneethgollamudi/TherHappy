...
import { saveMood, getTodayMood, getMoodHistory, getJournalEntries, calcStreak } from '@/lib/db';
import { getGreeting, getTodayAffirmation, getMoodConfig, MOOD_CONFIG } from '@/lib/utils';
import type { DbJournalEntry } from '@/lib/db';

const FEATURE_CARDS = [
  { href: '/chat', icon: MessageCircle, title: 'Talk with Aria', subtitle: 'AI-powered compassionate support', bg: 'from-lavender-50 to-purple-50', iconBg: 'bg-lavender-500' },
  { href: '/journal', icon: BookOpen, title: 'Journal', subtitle: 'Write freely, think clearly', bg: 'from-rose-50 to-pink-50', iconBg: 'bg-rose-400' },
  { href: '/breathe', icon: Wind, title: 'Breathe', subtitle: 'Guided breathing exercises', bg: 'from-teal-50 to-emerald-50', iconBg: 'bg-teal-500' },
  { href: '/insights', icon: BarChart2, title: 'Insights', subtitle: 'Track your mood journey', bg: 'from-amber-50 to-orange-50', iconBg: 'bg-amber-400' },
];

export default function HomePage() {
  const [selectedMood, setSelectedMood] = useState<number | null>(null);
  const [streak, setStreak] = useState(0);
  const [recentEntries, setRecentEntries] = useState<DbJournalEntry[]>([]);
  const [savedMood, setSavedMood] = useState(false);
  const [affirmation] = useState(getTodayAffirmation);

  useEffect(() => {
    getTodayMood().then(entry => {
      if (entry) { setSelectedMood(entry.mood); setSavedMood(true); }
    });
    getMoodHistory(60).then(history => setStreak(calcStreak(history)));
    getJournalEntries().then(entries => setRecentEntries(entries.slice(0, 2)));
  }, []);
  ...
  const handleMoodSelect = async (value: number) => {
    ...
    const history = await getMoodHistory(60);
    ...
    <div className="min-h-screen">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-6 pt-12 pb-8 md:pt-16">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-1">{getGreeting()} 👋</h1>
          ...
          <div className="glass rounded-2xl p-5 shadow-xl shadow-lavender-200/40">
            ...
            <div className="flex justify-around">
              {MOOD_CONFIG.map((mood) => (
                ...
                >
                  <span className="text-2xl md:text-3xl mood-emoji transition-transform duration-200">{mood.emoji}</span>
                  <span className={`text-[10px] font-medium ${selectedMood === mood.value ? mood.text : 'text-slate-400'}`}>{mood.label}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {FEATURE_CARDS.map(({ href, icon: Icon, title, subtitle, bg, iconBg }) => (
              <Link key={href} href={href} className={`feature-card bg-gradient-to-br ${bg} rounded-2xl p-4 border border-white/60 shadow-sm hover:shadow-md group`}>
          ...
          <section>
            ...
            <div className="space-y-3">
              {recentEntries.map((entry) => {
                ...
                return (
                  <Link key={entry.id} href={`/journal?id=${entry.id}`} className="flex items-start gap-3 glass rounded-xl p-4 hover:shadow-md transition-all group">
                    {moodCfg && <span className="text-2xl flex-shrink-0">{moodCfg.emoji}</span>}
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-slate-700 text-sm group-hover:text-lavender-700 transition-colors truncate">{entry.title || 'Untitled Entry'}</p>
                      <p className="text-slate-400 text-xs mt-0.5 truncate">{entry.content.slice(0, 80)}…</p>
                      <p className="text-lavender-400 text-xs mt-1.5">{new Date(entry.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</p>
                    </div>
          ...
          <section>
            <div className="glass rounded-2xl p-6 border border-lavender-100 text-center">
              ...
              <div className="flex gap-2 justify-center flex-wrap">
                <Link href="/chat" className="bg-lavender-600 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-lavender-700 transition-colors">Chat with Aria</Link>
                <Link href="/journal" className="bg-white text-lavender-700 text-sm font-medium px-4 py-2 rounded-xl border border-lavender-200 hover:bg-lavender-50 transition-colors">Write in Journal</Link>
              </div>
            </div>
          </section>
        )}
