...
const CHAT_STORAGE_KEY = 'th_chatMessages';
    ...
    <div className="flex items-end gap-2 animate-fade-in">
      ...
      <div className="bg-white border border-lavender-100 rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm">
        <div className="flex items-center gap-1">
          {[0, 1, 2].map(i => (
            <div key={i} className="typing-dot w-2 h-2 bg-lavender-400 rounded-full animate-typing" style={{ animationDelay: `${i * 0.2}s` }} />
          ))}
        </div>
      </div>
    </div>
...
export default function ChatPage() {
  ...
  const handleSend = async (content?: string) => {
    ...
    try {
      ...
      const assistantMessage: ChatMessage = { role: 'assistant', content: fullText, timestamp: new Date().toISOString() };
      const final = [...updated, assistantMessage];
      setMessages(final);
      localStorage.setItem(CHAT_STORAGE_KEY, JSON.stringify(final));
      setStreamingContent('');
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-shrink-0 bg-white/80 backdrop-blur-xl border-b border-lavender-100 px-4 md:px-8 py-4 flex items-center justify-between">
        ...
        <button onClick={handleClear} className="flex items-center gap-1.5 text-slate-400 hover:text-rose-500 text-sm font-medium transition-colors p-2 rounded-lg hover:bg-rose-50">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-4">
        ...
        {!isEmpty && (
          <>
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-end gap-2 animate-fade-in ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-8 h-8 bg-gradient-to-br from-lavender-500 to-pink-500 rounded-full flex items-center justify-center flex-shrink-0 shadow-md">
                    <Heart className="w-4 h-4 text-white" fill="white" />
                  </div>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm text-sm leading-relaxed whitespace-pre-wrap
                  ${msg.role === 'user' ? 'message-bubble-user text-white rounded-br-sm' : 'message-bubble-aria text-slate-700 rounded-bl-sm'}`}>
                  {msg.content}
                  {msg.timestamp && (
        ...
        {error && (
          <div className="flex items-start gap-2 bg-rose-50 border border-rose-200 rounded-xl p-4 animate-fade-in">
            ...
            <div>
              ...
              <button onClick={() => setError(null)} className="flex items-center gap-1 text-rose-600 hover:text-rose-700 text-xs font-medium mt-2">
